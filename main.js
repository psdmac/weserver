...
wss.sockets.on('connection', function(socket) {
    ...
    socket.on('geodata', function(data){route.onGeodata(socket, data);});
});

// router for web server
//web.get('/activate/account', route.activateAccount);
//web.get('/reset/password', route.resetPassword);

web.listen(config.port);

console.log("GeoROS listening on port %d in %s mode", wss.server.address().port, wss.server.settings.env);
